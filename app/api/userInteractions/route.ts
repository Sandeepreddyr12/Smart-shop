...
export const POST = async (request: NextRequest) => {
  try {
    ...
    const { userId, productId, interactionType, value, reviewStars, sessionId, searchQuery } = parsed.data
    ...
    const existingInteraction = await UserInteractions.findOne({ userId, productId })
    ...
    if (existingInteraction) {
      ...
      const newInteraction = {
        ...
        productId,
        interactionType,
        value: (interactionType === 'add_to_cart' || interactionType === 'purchase') ? value || 1 : undefined,
